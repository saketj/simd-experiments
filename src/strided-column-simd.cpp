...
template<class T>
void vectorizedColumnCompare(const int stride, const int n, const char * __restrict__ a, const char * __restrict__ b, char * __restrict__ out) {
  for(int i = 0; i < n; i++) { // Vectorized comparison.
    T val1 = a[i * stride]; 
    T val2 = b[i * stride];
    out[i] = val1 > val2 ? 0x1 : 0x0;
  }
}
...
double benchmark_trial(size_t num_threads, size_t n) {
  ...
  char **out_bitset = new char*[num_threads];

  for (size_t i = 0; i < num_threads; ++i) { 
    a[i] = new T[n];
    b[i] = new T[n];
    out_bitset[i] = new char[n];
  ...
  // Verify correctness.
  for (size_t i = 0; i < num_threads; ++i) {
    char *c_ptr = out_bitset[i];
    for (size_t j = 0; j < n; ++j) {
      bool is_set = (c_ptr[j] & 0x1) > 0;
      if (a[i][j] > b[i][j]) {
	assert(is_set == true);
      } else {
	assert(is_set == false);
...
void* thread_main(void *args) {
  ...
  char* a_ptr = (char *) init->a;
  char* b_ptr = (char *) init->b;
  ...
  vectorizedColumnCompare<T>(stride, init->n, a_ptr, b_ptr, init->out);
...
void output(vector<pair<size_t, double>> results, size_t type_size, size_t num_threads) {
  for (size_t i = 0; i < results.size(); ++i) {
    double memory_accessed_per_thread = ((results[i].first * type_size * 2.0) + (results[i].first)) / (1024.0 * 1024.0 * 1024.0);
...
struct thread_args {
  ...
  long begin;
  long end;
  thread_args(int _idx, int _n, T *_a, T *_b, char *_out) 
